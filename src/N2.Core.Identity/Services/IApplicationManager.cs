...
public interface IApplicationManager {
    ...
    Task<ICommandResponse> CreateAsync(Guid tenantId, [NotNull] Application application, CancellationToken token);

    /// <summary>Deletes a tenant and removes all user-tenant associations.</summary>
    /// <param name="tenantId">The tenant to which the application belongs.</param>
    /// <param name="application">The application to delete.</param>
    /// <param name="token">Cancellation token.</param>
    Task<ICommandResponse> DeleteAsync(Guid tenantId, [NotNull] Application application, CancellationToken token);
    ...
    /// <param name="token">Cancellation token.</param>
    Task<ICommandResponse> UpdateAsync(Guid tenantId, [NotNull] Application application, CancellationToken token);
    ...
    Task<Application?> FindByIdAsync(Guid tenantId, Guid applicationId, CancellationToken token);

    /// <summary>Finds an application by name within a tenant. The value is normalised internally before lookup.</summary>
    Task<Application?> FindByNameAsync(Guid tenantId, string name, CancellationToken token);

    /// <summary>Returns a select list of all applications belonging to the given tenant, suitable for UI rendering.</summary>
    Task<SelectItemList<UserSelectItem>> GetApplicationsAsync(Guid tenantId, CancellationToken token);
    ...
    Task<ICommandResponse> LockAsync(Guid tenantId, [NotNull] Application application, CancellationToken token);
    /// <summary>Unlocks a previously locked application.</summary>
    Task<ICommandResponse> UnlockAsync(Guid tenantId, [NotNull] Application application, CancellationToken token);
